...
x=len(X)%int(d) # check if "d" splits "n" equally
if x!=0:
   print("The number is NOT 'strictly increasing'")
   exit() #quit if "d" doesn't split "n" into equal parts

y=int(len(X)/int(d))#calculate the number of splits and store it in "y"
if y > 1:
   for z in range(0,len(X)-int(d),int(d)): #loop through all the splits
      split1=X[z:z+int(d)]
      split2=X[z+int(d):z+2*int(d)]
      if (split2 <= split1): #check if split1 is <= split2
         print("The number is NOT strictly increasing")
         exit() #quit if its not increasing

for a in range(0,len(X),int(d)): #if the code reaches this part, then the number is strictly increasing
   print(X[a:a+int(d)] , end=',') # print the splits separated by ","
